    ...
    return fetchUserInfo().then(userInfo => {
        ...
        return Promise.all(messageRequests)
            .then(results => {
                ...
                let leaderboardText = `**DM with ${otherUser.global_name}**\n**Total messages:** ${data.total_results}\n\nLeaderboard:\n`;
                results.forEach((result, index) => {
                    leaderboardText += `${index + 1}. **${result.username}**: ${result.messageCount} messages\n`;
                });

                return sendLeaderboardMessage(leaderboardText);
            });
    });
}
        ...
        return Promise.all(recipientRequests)
            .then(() => {
                ...
                messageCounts.forEach((messageData, index) => {
                    leaderboardText += `${index + 1}. **${messageData.username}**: ${messageData.messageCount} messages\n`;
                });
...
function sendLeaderboardMessage(leaderboardText) {
